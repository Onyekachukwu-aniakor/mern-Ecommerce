...
const Navbar = () => {
  ...
  return (
    ...
    <nav className='container mx-auto flex items-center py-4 px-6 justify-between'>
        ...
        <div className='flex items-center space-x-4'>
            ...
            <button onClick={toggleCartDrawer} className='relative hover:text-black'>
                ...
                {cartItemCount > 0 && (<span className='bg-[#ea2e0e] absolute -top-1 text-white text-xs 
                rounded-full px-2 py-0.5'>{cartItemCount}</span>)}
    ...
    {/* Mobile Navigation */}
    <div className={`fixed top-o left-0 w-3/4 sm:w-1/2 md:w-1/3 h-full bg-white shadow-lg transform transition-transform duration-300 z-50 ${navDrawerOpen ? 'translate-x-0' : '-translate-x-full'}`}>
